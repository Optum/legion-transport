...
require 'spec_helper'
require 'legion/transport/queues/task_update'

RSpec.describe Legion::Transport::Queues::TaskUpdate do
  it 'is a class' do
  ...
  it 'is defined under Legion::Transport::Queues' do
    expect(described_class.name).to eq 'Legion::Transport::Queues::TaskUpdate'
  end

  it 'returns the correct queue name' do
    instance = described_class.allocate
    expect(instance.queue_name).to eq 'task.update'
  end
  ...
  it 'includes the task dead-letter exchange in queue options' do
    instance = described_class.allocate
    dlx = instance.queue_options.dig(:arguments, :'x-dead-letter-exchange')
    expect(dlx).to eq 'task.dlx'
  end
end
